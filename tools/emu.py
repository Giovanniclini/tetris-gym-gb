...
class Tetris:
    ...
    def to_title(self):
        """Boot through both copyright screens to the title screen."""
        self.run_until_state(GS_TITLE_SCREEN_MAIN)

    def to_level_select(self, hearts=False):
        """Title -> game type -> music -> A-TYPE level select.

        `hearts` holds Down while confirming on the title screen, which is how
        the original arms hard mode (see docs/research.md 3.7).
        """
        self.to_title()
        if hearts:
            self.hold("down")
        ...
        self.press("start")                       # accept A-TYPE
