...
The Gym had been hiding half of this. It clamps `hATypeLevel` back into the grid
...
routine is then correct for levels 0–22 with no change to it at all. Gym state
moves up to `$D8C1`.

The clamp stays, but the score is filed before it: `GymLevelSelectInit` calls
...
* **`$D762–$D8C0` was never really free.** It was reachable from unmodified
  ...
  landed on `$D800`, which is where Gym state used to start.
* **The panel routine no longer borrows `hATypeLevel`.** `GymUpdateHighScores`
...
* **Tests read Gym addresses from the `.sym` file** (`tools.emu.sym`) rather
