...
| 15 | `code/bank_000.s` | `IF GYM` routes the `$25` jump-table entry (copyright wait) through `GymStateHook`, which goes straight to the title screen | 3 (hook) | The copyright screen waits `$FA` frames, sets another `$FA`, and only then accepts a button: 8.5 s before the title on every boot. `$24` still runs, so the tile data and the demo piece table are still loaded; only the timer is cut. Boot to title goes from 9.8 s to 1.4 s. | 2. Zero when `GYM=0`. |
